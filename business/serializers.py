...
from .models import Business, KnowledgeBase, EscalationDepartment, Product, Category, OrderProduct, \
    Order
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            "id", "order_id", "delivery_info", "payment_summary", 
            "delivery_date", "status", "customer_id", "business", "customer",
            "order_products", "order_products_data", "date_created", "last_updated"
        ]
        read_only_fields = ["id", "order_id", "business", "date_created", "last_updated", "customer"]
    ...
    def _validate_and_reserve_inventory(self, order_products_data, business):
        ...
        for product_data in order_products_data:
            ...
            inventory_updates.append({
                'product': product,
                'quantity_to_deduct': stock_quantity
            })
    ...
    def create(self, validated_data):
        ...
        inventory_updates = self._validate_and_reserve_inventory(order_products_data, default_business)
    ...
    def update(self, instance, validated_data):
        ...
        # Handle status change from CANCELLED to active status - deduct inventory again
            ...
            if order_products_data:
                ...
                inventory_updates = self._validate_and_reserve_inventory(order_products_data_from_existing, default_business)
